...
//     const llvm::SmallDenseMap<Value, PtrState> &known) {
//   // Any inserted instruction should be before this yield
//   OpBuilder::InsertionGuard insertionGuard{rewriter};
//   rewriter.setInsertionPoint(op);
...
//     PtrState state;
//     if (reintCastOp) {
//       visitOperandReintCast(reintCastOp, state, op.getLoc(), rewriter, known);
//     } else {
//       visitOperand(v, state, op.getLoc(), rewriter, known);
//     }
//     initArgState.push_back(state);
//   }
...
// void PtrAnalysis::rewriteForOp(
//     scf::ForOp op, ConversionPatternRewriter &rewriter,
//     llvm::SmallDenseMap<Value, PtrState> &known) {

//   SmallVector<Value> newInitArgs;

//   SmallVector<std::pair<int, PtrState>, 5> initArgIndexIfPtrState;
//   SmallVector<std::pair<int, PtrState>, 5> knownPtrsTmp;
...
//     PtrState data;
//     if (reintCastOp) {
//       visitOperandReintCast(reintCastOp, data, op.getLoc(), rewriter,
//                            llvm::SmallDenseMap<Value, PtrState>(0));
//     } else {
//       visitOperand(arg, data, op.getLoc(), rewriter,
//             llvm::SmallDenseMap<Value, PtrState>(0));
//     }

//     // Record the BlockData for later processing
//     initArgIndexIfPtrState.push_back(std::make_pair(i, data));
//   }
...
//   // instructions to describe offset and stride for each dimension and append
//   // them to init args
//   for (auto [i, data] : initArgIndexIfPtrState) {
//     // For each dimension, if the corresponding offset and stride is an
//     // integer attribute, create a constant value and append them at the
