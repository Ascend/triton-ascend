...
//#include "Dialect/TritonStructured/IR/TritonStructuredOps.h.inc"
...
void MakeTensorPtrOp::build(OpBuilder &b, OperationState &state, Value base,
  ...
  // [BEG] origin
  // non-block pointer
  // Revert to the original version, now supports !ptr<tensor> conversion
  if (order.empty()) {
    resType = RankedTensorType::get(sizes, basePtr);
  }
  // block pointer
...
void LoadOp::build(OpBuilder &b, OperationState &state, Value ptr,
                   ArrayRef<OpFoldResult> dims, ArrayRef<int64_t> dim_mode, Value other) {
  ...
  build(b, state, resType, ptr, dynamicDims, b.getDenseI64ArrayAttr(staticDims), dim_mode,
        other);
}
...
// let builders = [
//     OpBuilder<(ins "RMWOpAttr":$atomic_rmw_op, "Value":$ptr, "Value":$value, 
// "ArrayRef<OpFoldResult>":$dims, "MemSemanticAttr":$sem, "MemSyncScopeAttr":$scope)>,
//   ];
//  ::mlir::TypeRange resultTypes,
//
void AtomicRMWOp::build(::mlir::OpBuilder &b, ::mlir::OperationState &state, ::mlir::Type type,
  ::mlir::triton::RMWOpAttr atomic_rmw_op, ::mlir::Value ptr, ::mlir::Value value, ArrayRef<OpFoldResult> dims,
  ::mlir::triton::MemSemanticAttr sem, ::mlir::triton::MemSyncScopeAttr scope) {
    SmallVector<int64_t> staticDims;
    SmallVector<Value> dynamicDims;
    

    dispatchIndexOpFoldResults(dims, dynamicDims, staticDims);
    build(b, state, type, atomic_rmw_op,  ptr, value, dynamicDims, b.getDenseI64ArrayAttr(staticDims), sem, scope );
}


// void AtomicRMWOp::build(OpBuilder &b, OperationState &state,  Value atomic_rmw_op, Value sem, Value scope,
...
//   build(b, state,  atomic_rmw_op, sem, scope,  ptr, value, dynamicDims, b.getDenseI64ArrayAttr(staticDims));
// }
