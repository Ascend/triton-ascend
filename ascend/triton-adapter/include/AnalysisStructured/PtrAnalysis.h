...
struct MemAccType {
  ...
  void merge(const MemAccType &other) {
...
// Get the computed PtrState for the forOp's init-arg at the provided index.
  ...
  LogicalResult rewriteDescriptorLoadOp(triton::DescriptorLoadOp op);
  LogicalResult rewriteDescriptorStoreOp(triton::DescriptorStoreOp op);
  ...
  LogicalResult walk(Operation *rootOp) ;
  ...
  LogicalResult generateMask(Operation * op, PtrState &ptrState, SmallVector<OpFoldResult> &dims, SmallVector<int64_t> &dimMode);

  LogicalResult checkModifiedByAddPtrConverter(triton::LoadOp &op) const;

  LogicalResult  continueModifyFromAddPtrConverter(triton::LoadOp &op ) ;
  ...
  LogicalResult rewriteAddPtrToUnstrucMemAcc(triton::AddPtrOp op,  PtrState &state) ;
