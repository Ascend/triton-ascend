...
static Value parseScalarValue(Value operand, Location loc,
                            ConversionPatternRewriter &rewriter) {
  if (isa<IntegerType>(operand.getType())) {
    return operand;
  }
  if (auto op = operand.getDefiningOp<triton::BroadcastOp>()) {
    return parseScalarValue(op.getSrc(), loc, rewriter);
  }else if (auto op = operand.getDefiningOp<triton::SplatOp>()) {
    return parseScalarValue(op.getSrc(), loc, rewriter);
  }else if (auto op = operand.getDefiningOp<triton::ExpandDimsOp>()) {
    return parseScalarValue(op.getSrc(), loc, rewriter);
  }else if (auto tensorType = dyn_cast<mlir::RankedTensorType>(operand.getType())) {
    auto index = rewriter.create<mlir::arith::ConstantIndexOp>(loc, 0);
    SmallVector<mlir::Value> indices;
    for(size_t i = 0; i < tensorType.getRank(); ++i){
      if(tensorType.getDimSize(i) != 1){
        InFlightDiagnostic diag = emitError(loc)
                                    << "assert does not support non-single elements";
        return nullptr;
      }
      indices.push_back(index);
    }
    auto extractedElement = rewriter.create<mlir::tensor::ExtractOp>(loc, operand, indices);
...
struct AssertConverter : public OpConversionPattern<triton::AssertOp> {
  ...
  matchAndRewrite(triton::AssertOp op, OpAdaptor adaptor,
    ...
    //   auto scalarVal = parseScalarValue(condVal, op.getLoc(), rewriter);
    ...
    //   llvm::dbgs() << "\033[34m" << "condVal.getType():" << condVal <<  "\n\033[0m";
    // }
